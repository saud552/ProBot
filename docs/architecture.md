...
- توحيد الاعتماد على MySQL كمخزن أساسي مع ملفات JSON اختيارية للنسخ الاحتياطي أو التخزين المؤقت الخفيف.
...
3. **Infrastructure Layer (`src/Infrastructure`)**
   - `Database`: موصل MySQL (PDO) مع فئات Repository (مثل `UserRepository`, `OrderRepository`).
...
- **MySQL**: قاعدة بيانات واحدة تجمع جميع الجداول المطلوبة من النظامين السابقين.
...
- سيتم توفير سكربت `setup/schema.sql` في المرحلة التالية لإنشاء الجداول وضمان التوافق مع البيانات الحالية.
...
2. تعريف ملف الإعدادات (MySQL، مفاتيح API) عبر `.env` (سيُضاف لاحقاً).
3. نقل الترجمات، ثم بناء `LanguageManager` الجديد.
4. إعداد طبقة الاتصال بـMySQL مع مستودع تجريبي.
