## Unified Data Model (SQLite + JSON Fallback)

> يوضح هذا المستند مخطط البيانات الأساسي للبوت الموحد، ويكمل ما جاء في `architecture.md`. يعتمد النظام على SQLite كمصدر رئيسي مع إمكانية حفظ نسخ JSON احتياطية (مثل `storage/backups/*.json`) أو استخدام `JsonStore` لبيانات مؤقتة صغيرة.

### المبادئ
- **فصل الاهتمامات**: كل جدول مسؤول عن مفهوم واحد فقط (Users، Wallets، Orders، Services ...).
- **التناسق**: جميع العمليات الحرجة (خصم الرصيد، إنشاء طلب) تمر عبر معاملات SQLite (PDO + PRAGMA foreign_keys).
...
- **الترحيل**: يعتمد المشروع على سكربت SQLite واحد `setup/schema.sql` لإنشاء القاعدة نظيفة، ويمكن إضافة سكربتات ترحيل لاحقة لقراءة بيانات Number/TigerSpeed ونقلها.
